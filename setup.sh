...
# - Sets up braindump folder for incubation materials
            ...
            -d @- << PAYLOAD
{
  ...
  "children": [
    ...
    {"object": "block", "type": "paragraph", "paragraph": {"rich_text": [{"type": "text", "text": {"content": "Status: Ideating | Phase: 0 - INCUBATE | Last touched: $TODAY"}}]}},
    {"object": "block", "type": "paragraph", "paragraph": {"rich_text": [{"type": "text", "text": {"content": "Next action: Run /incubate or skip to Phase 1"}}]}},
...
└── incubation-context.md   # (Generated by Phase 0 - structured for /capture)
```

## Processing

When ready, tell Claude: "Run /incubate"

This triggers Phase 0 (Cognitive Incubation):
1. **Material Absorption** - Extract explicit and implicit ideas
...
4. **Creative Synthesis** - AI builds deep understanding
5. **Write Context** - Saves structured output to incubation-context.md

After processing:
- `incubation-context.md` contains parsed insights for Phase 1
- `status.json` is updated to mark Phase 0 complete
- /capture will **automatically load** this context to inform questions
...
cat > status.json << EOF
{
  ...
  "status": "initialized",
  "phases": {
    "0-incubate": { "status": "pending", "optional": true },
...
[Add your initial thoughts here - or run incubation first with /incubate]
...
- Ready to begin Phase 0 (INCUBATE) or Phase 1 (CAPTURE)
EOF
...
| Phase | Name | Purpose |
|-------|------|---------|
| 0 | INCUBATE | Cognitive incubation from materials and prior projects (optional) |
